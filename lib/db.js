...
db.checkRate = function (actorKey, limitPerMin) {
  if (limitPerMin === undefined) limitPerMin = 5;
  const k = 'rate:' + actorKey + ':' + Math.floor(Date.now() / 60000);
  const c = (__rateMap.get(k) || 0) + 1;
  if (c > limitPerMin) return false;
  __rateMap.set(k, c);
  if (__rateMap.size > 500) {
    const nowMin = Math.floor(Date.now() / 60000);
    for (const [key] of __rateMap) {
      const m = Number(key.split(':')[2]);
      if (nowMin - m > 30) __rateMap.delete(key);
    }
  }
  return true;
};
